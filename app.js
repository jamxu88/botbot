...
client.on("message", (message) => {
	...
	if (message.attachments.size > 0) {
		console.log(message.member.user.tag + ': "' + message.content + messageattachment.url + '" in channel #' + message.channel.name);
		client.channels.get("671414066157191185").send('> **' + message.member.user.tag + '**: "' + message.content + messageattachment.url + '" in channel **#' + message.channel.name + '**')
	}else {
